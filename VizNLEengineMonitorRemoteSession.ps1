...
[int] $port = 6100
$engine = "localhost" # Hostname or IP address
...
try
{
    ...
    $remoteparam=@{}
    $remoteparam = @{ computername=$engine; credential=$credential}
    Invoke-Command @remoteparam {
	...
	} # Invoke-Command does not work in this case because it connects to session 0 which cannot load a GUI into session 1
}
catch
{
    $Error
